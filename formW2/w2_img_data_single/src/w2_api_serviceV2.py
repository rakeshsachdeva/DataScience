...
def validate_response(responsedata,inpfilename,uniqid,docid):
   ...
   #validated_res["uniqueId"],validated_res["uniqueIdStatus"]=w2_str_features(uniqid,mincharlimit=17)
    validated_res["employerName"],validated_res["employerNameStatus"]=w2_str_features(responsedata["employer name"],mincharlimit=3)
    validated_res["employeeName"],validated_res["employeeNameStatus"]=w2_str_features(responsedata["employee name"],mincharlimit=5)
    ...
    validated_res["employerIdNumber"],validated_res["employerIdNumberStatus"]= empid_response(responsedata["employer id number"])
    validated_res["wagesTipsOtherComp"],validated_res["wagesTipsOtherCompStatus"]=wages_response(responsedata["wages tips other comp"])
    validated_res["socialSecurityWages"],validated_res["socialSecurityWagesStatus"]=wages_response(responsedata["social security wages"])
    validated_res["medicareWageAndTips"],validated_res["medicareWageAndTipsStatus"]=wages_response(responsedata["medicare wages and tips"])
    ...
    elif validated_res["employeeNameStatus"] == "Pass" and validated_res["wagesTipsOtherCompStatus"] == "Pass" and validated_res["yearStatus"] == "Pass":
        validated_res["docStatus"] = "Success"
    elif (validated_res["employeeNameStatus"] =="Pass" and validated_res["wagesTipsOtherCompStatus"] =="Pass") or (validated_res["employeeNameStatus"] and validated_res["yearStatus"] =="Pass") or (validated_res["wagesTipsOtherCompStatus"] =="Pass" and validated_res["yearStatus"] =="Pass"):
...
@cross_origin()
def processw2Api():
    inputRequestFomat={"data":[{"filePath":"w2_file_path","documentId":1001},{"filePath":"w2_file_path2","documentId":1002}]}
    ...
    if not request.is_json :
        apireponse={"status":  103, "error": "Content_Type should be applicatin/json,Expecting json data key as : "+str(inputRequestFomat)}
        Merge(apireponse,{})
        return json.dumps({"data":[apireponse]})
    ...
    for eachrequest in request.json['data']:
        if type(eachrequest)!=dict or ('filePath' not in eachrequest) or ('documentId' not in eachrequest):
            apireponse = {"status": 105,"error": "Expecting json, 'data' as key eg : "+str(inputRequestFomat)}
            Merge(apireponse, {})
            return json.dumps({"data": [apireponse]})
        elif type(eachrequest)==dict and 'filePath'  in eachrequest and 'documentId'  in eachrequest :
            filenames.append(eachrequest['filePath'])
            documentIds.append(eachrequest['documentId'])


    if len(filenames)!=len(documentIds):
        apireponse = {"status": 106,"error": "Expecting json,with equal number 'documentid' and 'filePath' in data key  : "+str(inputRequestFomat)}
        Merge(apireponse, {})
        return json.dumps({"data": [apireponse]})
